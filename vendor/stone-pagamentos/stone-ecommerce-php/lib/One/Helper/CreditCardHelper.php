...
abstract class CreditCardHelper
{
    ...
    {
        if (empty($number)) {
            throw new CreditCardError("Número do cartão de crédito é inválido.", "number");
        }

        if (empty($name)) {
            throw new CreditCardError("Nome do titular do cartão de crédito inválido.", "holderName");
        }

        if (empty($cvc)) {
            throw new CreditCardError("Código de segurança do cartão de crédito inválido.", "securityCode");
        }

        // Verifica se foi enviado uma barra
        if (empty($expiry) || stristr($expiry, '/') === false) {
            throw new CreditCardError("Data de validade do cartão de crédito inválido.", "expirationDate");
        }
        ...
        if ($expMonth < 1 || $expMonth > 12) {
            throw new CreditCardError("Mês de validade do cartão de crédito inválido.", "expirationMonth");
        }

        // Verifica se o ano é válido
        if (!in_array(strlen($expYear), array(2, 4))) {
            throw new CreditCardError("Ano de validade do cartão de crédito inválido.", "expirationYear");
        }
        ...
        if (strlen($number) < 10 || strlen($number) > 24){
            throw new CreditCardError("Número do cartão de crédito inválido.", "number");
        }
        ...
        if ($creditCardBrand == null) {
            throw new CreditCardError("Marca do cartão de crédito inválida.", "brand");
        }
